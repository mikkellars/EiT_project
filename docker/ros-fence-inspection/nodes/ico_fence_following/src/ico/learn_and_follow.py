...
class LearnFollow():
    ...
    def one_ico_learning(self, cur_ang, reflective, predictive, sim:bool = True, mc_scaling:float = 1.0):
        ...
        if (cur_ang < -80) and (reflective == -1): #reflective == -1:
            print('right turn')
            right_mc_val = 0.0
            if sim:
                left_mc_val = 0.2
                ...
                left_mc_val = 20.0 
        elif (cur_ang > -100) and (reflective == 1): #reflective == 1:
            print('left turn')
            left_mc_val = 0.0
            if sim:
                right_mc_val = 0.2
    ...
    def two_ico_learning(self, cur_ang, reflective, predictive, sim:bool = True, mc_scaling:float = 1.0):
    ...
    def __callback(self, msg):
       ...
       # reflex, predictive = self.__detect_relfex_ang(cur_dist, cur_ang)
        ...
        if self.learn_type is 'one':
            right_mc_val, left_mc_val = self.one_ico_learning(reflex, predictive, sim=False, mc_scaling=100.0)
        elif self.learn_type is 'two':
            right_mc_val, left_mc_val = self.two_ico_learning(cur_ang, reflex, predictive, sim=False, mc_scaling=50.0)
        else:
            ValueError('Learning type only supports one or two')
